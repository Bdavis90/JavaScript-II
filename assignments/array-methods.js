...
runners.forEach((name, index) => {
    fullName.push(name.first_name + ' ' + name.last_name + ' ' + index);
})
...
let caps = runners.map(names => names.first_name.toUpperCase());
allCaps.push(caps);
console.log(allCaps);
...
let shirts = runners.filter(size => size.shirt_size === 'L');
